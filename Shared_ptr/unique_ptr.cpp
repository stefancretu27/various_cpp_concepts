...
int main()
{
    ...
    unique_ptr<double> up_default_created;

    if(up_default_created)
    {
        cout<<"Default created unique_ptr is NOT null"<<endl;
    }
    else
    {
        cout<<"Default created unique_ptr is null"<<endl;
    }
    

    up_default_created = unique_ptr<double>{new double{3.1415}};
    cout<<"after unique_ptr was default created, it is set to point to some heap memory as it is assigned an r-value returned by unique_ptr c-tor call "<<*up_default_created<<endl;
    ...
    cout<<"     3. A unique_ptr that is not set to point to some object upon creation, automatically is set to nullptr. After creation, it can be later set to point to an object by"<<endl;
    cout<<"     using move semantics or by assigning to it the anonymous object returned by a call to unique_ptr c-tor, that is an r-value, which leads to unique_ptr move c-tor call."<<endl<<endl;
    cout<<"     4. A class member unique_ptr can be initialized either in member initialization list, using new or make_unique, either later, only using move semantics."<<endl<<endl;
    cout<<"     5. Returning unique_ptr from function/method can be done either by reference, with the returned value stored in a reference variable, thus avoid copy c-tor calls,"<<endl;
    cout<<"     either using move semantics, with the returned value stored in a reference to r-value."<<endl<<endl;
    cout<<"     6. A unique_ptr can be passed as argument to a function by reference, showing the intention to use an existing unique_ptr and the possibilty to change/redirect it"<<endl;
    ...
    cout<<"     7. Having a reference to r-value to a unique_ptr, means that an existing unique_ptr can be 'moved' to it. However, the existing pointer is not left in a stable/null"<<endl;
    ...
    cout<<"     8. A unique_ptr can be passed by value as argument to a function. This implies moving the unique_ptr as copy c-tor cannot be called. As a result, the resource"<<endl;
    ...
    cout<<"     9. Likewise having a reference to unqiue_ptr does not entail creating a new object, so it happens when having reference to r-value unique_ptr. It just creates a"<<endl;
